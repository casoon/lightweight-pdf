...
use lightweight_pdf::*;
use support::{page_count, pdftotext_page as page_text};
...
fn page_break_element_forces_a_new_page() {
    ...
    assert!(page_text(&bytes, 1).contains("Seite eins"));
    assert!(!page_text(&bytes, 1).contains("Seite zwei"));
    assert!(page_text(&bytes, 2).contains("Seite zwei"));
}
...
fn header_and_footer_are_suppressed_on_a_cover_page() {
    ...
    let bytes = doc.render().unwrap();
    assert!(!page_text(&bytes, 1).contains("Firmenkopf"), "cover page must not show the header");
    assert!(page_text(&bytes, 2).contains("Firmenkopf"), "page 2 must show the header");
}
